...
class release_info:
    ...
    def __init__(self, title_id):
        ...
        returns: tags
        """
        releases = catch(lambda: table_tag[index_finder(
        ...
        try:
            ...
            for tag in releases:
                self.releases_df.loc[len(self.releases_df)] = [catch(lambda: unicode(tag.select_one('td.release-date-item__country-name').a.get_text())),
                                                               catch(lambda: "%s%s" % (base_uri, unicode(tag.select_one(
                                                                   'td.release-date-item__country-name').a['href'][1:]))),
                                                               catch(lambda: unicode(tag.select_one(
                                                                   'td.release-date-item__date').get_text())),
                                                               catch(lambda: unicode(tag.select_one('td.release-date-item__attributes').get_text()))]
        ...
        """
        self.released_country_names = catch_list(
            lambda: self.releases_df.Country.tolist())
        self.released_country_uri = catch_list(
            lambda: self.releases_df.URI.tolist())
        self.released_dates = catch_list(
            lambda: self.releases_df.Date.tolist())
        self.released_locations = catch_list(
            lambda: self.releases_df.Location.tolist())

        """
        :returns: Released Date in India if available.
        """
        self.release_date_in_india = catch(lambda: unicode(releases[india_index_finder(
        ...
        try:
            ...
            for tag in aka:
                self.also_known_as_df.loc[len(self.also_known_as_df)] = [catch(lambda: unicode(tag.select_one('td.aka-item__name').get_text())),
                                                                         catch(lambda: unicode(tag.select_one('td.aka-item__title').get_text()))]
        ...
        """
        self.also_known_as_country_names = catch_list(
            lambda: self.also_known_as_df.Country.tolist())
        self.also_known_as_titles = catch_list(
            lambda: self.also_known_as_df.Title.tolist())

        """
        :returns: Creates Meta Data from the above info. if available.
        """
        self.imdb_release_info_metadata = catch_dict(lambda: {"Movie Name": self.title,
