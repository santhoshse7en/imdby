...
from imdb.utils.helpers import (catch, catch_dict, catch_list,
                                     dataframe_data, unicode, company_data)
...
class company:
    ...
    def __init__(self, title_id):
        ...
        self.production_company_df = catch(
            lambda: company_data(tag_search['production'], soup))
        self.production_company_name = catch_list(
            lambda: self.production_company_df.Name.tolist())
        self.production_company_id = catch_list(
            lambda: self.production_company_df.ID.tolist())
        self.production_company_uri = catch_list(
            lambda: self.production_company_df.URI.tolist())
        """
        :returns: Distributors DataFrame
        """
        self.distributors_df = catch(
            lambda: company_data(tag_search['distributor'], soup))
        self.distributors_name = catch_list(
            lambda: self.distributors_df.Name.tolist())
        self.distributors_id = catch_list(
            lambda: self.distributors_df.ID.tolist())
        self.distributors_uri = catch_list(
            lambda: self.distributors_df.URI.tolist())

        """
        :returns: Special Effects DataFrame
        """
        self.specials_effects_df = catch(
            lambda: company_data(tag_search['special_effects'], soup))
        self.specials_effects_name = catch_list(
            lambda: self.specials_effects_df.Name.tolist())
        self.specials_effects_id = catch_list(
            lambda: self.specials_effects_df.ID.tolist())
        self.specials_effects_uri = catch_list(
            lambda: self.specials_effects_df.URI.tolist())

        """
        :returns: Other Companies DataFrame
        """
        self.other_companies_df = catch(
            lambda: company_data(tag_search['other'], soup))
        self.other_companies_name = catch_list(
            lambda: self.other_companies_df.Name.tolist())
        self.other_companies_id = catch_list(
            lambda: self.other_companies_df.ID.tolist())
        self.other_companies_uri = catch_list(
            lambda: self.other_companies_df.URI.tolist())

        """
        returns: Creates Dict from the above info. if available.
        """
        self.imdb_company_metadata = catch_dict(lambda: {"Movie Name": self.title,
